...
export const addWishList = (axiosInstance, form) => {
  ...
  return axiosInstance.post(`/api/v1/user/addwishlist?id=${id}&projectID=${projectID}`);
};

export const deleteWishlist = (axiosInstance, form) => {
  const id = form.id;
  const projectID = form.projectID;
  return axiosInstance.delete(`/api/v1/user/deletewishlist?id=${id}&projectID=${projectID}`);
};

export const checkWishLish = (axiosInstance, form) => {
  const id = form.id;
  const projectID = form.projectID;
  return axiosInstance.get(`/api/v1/user/checkprojectwishlist?id=${id}&projectID=${projectID}`);
};

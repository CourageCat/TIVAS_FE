...
import { addWishList, checkWishLish,deleteWishlist } from "~/controllers/user";
...
function ProjectDetail() {
  ...
  useEffect(() => {
    const projectDetail = async () => {
      ...
      if (currentUser) {
        const res_wishList = await checkWishLish(axiosInstance, {
  ...
  const handleChangeChecked = async (event) => {
    if (!currentUser) {
      return toast.custom(() => (
        <ToastNotify type="warning" title="Warning" desc="Please log in" />
      ));
    }
    ...
    if (event.target.checked === true) {
      ...
      toast.custom(() => (
        <ToastNotify type="success" title="success" desc="Added to wishlist" />
      ));
    } else {
